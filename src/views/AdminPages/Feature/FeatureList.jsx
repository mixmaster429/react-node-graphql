...
import {GET_ADMIN_FEATURELIST_DETAILS,DELETE, GET_TOGGLE_STATUS, UPDATE_TOGGLE_STATUS} from "../../../queries";
...
class FeatureList extends React.Component {
  ...
  dataMapper(props) {
  var {getAdminFeatureDetails} = props;
    if (getAdminFeatureDetails && getAdminFeatureDetails.getAdminFeaturedDetails) {
      let test = getAdminFeatureDetails.getAdminFeaturedDetails;
        this.setState({
          data: test.map((feature, key) => {
            ...
            return {
              id: feature.id,
              name: c1[0],
              price: getSymbol(feature.currencySymbol)+feature.price,
                ...
                <div className="actions-right">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (obj.id) {
                        this.props.history.push(`/admin/featureList-editFeature/${obj.id}`);
                      }
                    }}
                    color="twitter"
                    className="edit"
  ...
  componentWillMount() {
    let {getAdminFeatureDetails} = this.props;
    getAdminFeatureDetails.refetch({fetch: "all"});
    if (getAdminFeatureDetails) {
  ...
  render() {
    let {currentChosen, dataToexport} = this.state;
          ...
          <Button
            ...
            onClick={(e) => {
              ...
              exportData(dataToexport ? dataToexport : this.reactTable.current.getResolvedState().sortedData.slice(0, 10), keys)
                ...
                <Button
                onClick={() => {this.props.history.push("/admin/featureList-addFeature");}}
                color="success"
                ...
                if (this.state.canDelete) {
                  ...
                  data.find((o, i) => {
                    if (o.id === currentChosen.id) {
                      this.props.delete({
                        variables: { id: Number(currentChosen.id), typeConfig: "featured" }
                      }).then(async (d) => {
                        if(d.data.delete) {
                          this.props.getAdminFeatureDetails.refetch({fetch: "all"});
                      //data.splice(i, 1);
                      this.props.updateToggle({
                        variables: {
                          toggleStatus: true,
...
var fList = compose(
  graphql(GET_ADMIN_FEATURELIST_DETAILS, {name: "getAdminFeatureDetails",options: () => ({variables: {fetch: "all"}})}),
